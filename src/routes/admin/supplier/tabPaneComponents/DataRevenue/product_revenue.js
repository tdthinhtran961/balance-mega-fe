...
const ProductRevenue = ({ subOrgId }) => {
  ...
  useEffect(() => {
    const initDataCategory = async () => {
      ...
      try {
        const res = (subOrgId === 'null' || subOrgId === 'undefined')
          ? await CategoryService.get({})
          : await CategoryService.get({ subOrgId: subOrgId })
      ...
      if (idCategory.idCategoryMain) {
        ...
        try {
          const res = (subOrgId === 'null' || subOrgId === 'undefined')
            ...
            : await CategoryService.get({ id: idCategory.idCategoryMain, subOrgId: subOrgId })
      ...
      if (idCategory.idCategory1) {
        ...
        try {
          const res = (subOrgId === 'null' || subOrgId === 'undefined')
            ...
            : await CategoryService.get({ id: idCategory.idCategory1, subOrgId: subOrgId })
  ...
  const [handleChangeDataRebvenueByProduct, DataRevenueByProduct] = HookDataTable({
    ...
    rightHeader: (
      <div className="flex items-start xl:flex-row flex-col 2xl:gap-3 mt-4">
        <div className="flex flex-row xl:mb-0 mb-4 w-full sm:w-fit">
          <div className="flex flex-col w-full">
            <Select
              className="sm:w-[195px] rounded-[10px] mr-0 sm:mr-4"
        ...
        <div
          className={classNames('sm:relative xl:my-0 mt-0', {
        ...
        >
          <div className="sm:flex gap-3 items-end justify-end flex-col md:flex-row">
            <Space
              direction="vertical"
              className="flex items-center gap-2 lg:w-[235px] sm:justify-between justify-start sm:mb-0 mb-4"
            >
            ...
            >
              <p className="text-[12px] text-left w-auto lg:w-[61px]">Đến ngày</p>{' '}
              <DatePicker
  ...
  return (
    <Fragment>
      <div className="revenueByProductAdmin">
