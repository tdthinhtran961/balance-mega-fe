...
const DataProductTransfer = ({ item, dataTempChoose, setDataTempChoose, remainingListProduct }) => {
  const [showTick, setShowTick] = useState(false);
  
  const handleChoose = (choosingItem) => {
    const indexChoosedDisposalGoods = dataTempChoose?.findIndex(
      (goods) => (goods.productId || goods.idProduct || goods.id) === choosingItem.productId,
    );
    if (indexChoosedDisposalGoods !== -1) {
      dataTempChoose.splice(indexChoosedDisposalGoods, 1);
      setShowTick(false);
    } else {
      dataTempChoose.push({ ...choosingItem, check: true });
      setShowTick(true);
    }
...
export default DataProductTransfer
