...
const PromotionGoodsCard = ({ item, dataTempChoose, setDataTempChoose, pageType }) => {
  const [showTick, setShowTick] = useState(false);
  const handleChoose = (itemChoose) => {
    const indexChoosedPromotionGoods = dataTempChoose?.findIndex((goods) => goods.id === itemChoose.id);
    if (indexChoosedPromotionGoods !== -1) {
      dataTempChoose.splice(indexChoosedPromotionGoods, 1);
      setShowTick(false);
    } else {
      setShowTick(true);
      dataTempChoose.push({ ...itemChoose, check: true });
    }
  ...
  return (
    <div
      className={` ${
        pageType === 'edit' ? 'w-[30%]' : ' w-[28%]'
      } bg-white rounded-[10px] shadow-xl hover:bg-gray-100 transition-all cursor-pointer relative h-[196px] text-center card-goodsReturn-item`}
    ...
    >
      <div className="flex flex-col justify-center items-center pt-[10px]" style={{ height: 'fit-content' }}>
        ...
        <Tooltip placement="topLeft" title={item?.name} mouseEnterDelay={0.1} mouseLeaveDelay={0.2}>
          <p className="max-w-[80%] text-ellipsis overflow-hidden whitespace-nowrap text-sm text-gray-600 font-normal mt-2">
            {item?.name}
          </p>
        </Tooltip>
...
export default PromotionGoodsCard;
