...
const ProductRevenue = ({ tabKey }) => {
  ...
  }, [filterDate, filterSupplier, filterStore, filterStatus, filterCategory]);
  ...
  const handelTesst = async () => {
    ...
    XLSX.utils.sheet_add_aoa(ws, [['Chọn của hàng:', headExcelInfo.storeName]], { origin: 'D5' });
    XLSX.utils.sheet_add_aoa(ws, [['Chọn nhà cung cấp:', headExcelInfo.supplierName]], { origin: 'G5' });
  ...
  const [handleChangeDataRebvenueByOrder, DataRevenueByProduct] = HookDataTable({
    ...
    rightHeader: (
      <div className="flex 2xl:items-end items-start flex-col 2xl:gap-3 2xl:mt-0 mt-3">
        <div className="flex 3xl:my-0 lg:flex-row flex-col 3xl:mb-0 md:mb-4 w-full sm:w-fit md:gap-2 lg:gap-0 ">
          <div className="flex flex-col">
            <Select
              className="lg:w-[195px] w-full rounded-[10px] mr-0 sm:mr-6 md:mb-0 mb-2"
            ...
            <div>{!filterStore && <span className="text-white"></span>}</div>
          </div>
          <div>
            <Select
              className="lg:w-[195px] w-full rounded-[10px] mr-0 sm:mr-6 md:mb-0 mb-2"
              ...
              onChange={async (event, options) => {
                ...
                setFilterSupplier();
                const supplierListBalance =
                  event &&
                  (await SupplierService.getListSupplierBalance({
                    page: 1,
                    perPage: 100000,
                    idSuppiler: event,
                  }));
                const supplierListNonBalance =
                  event &&
                  (await SupplierService.getListSupplierNonBal({
                    page: 1,
                    perPage: 100000,
                    storeId: event,
                    supplierType: 'NON_BALANCE',
                  }));
                event && setSupplierList(supplierListBalance?.data.concat(supplierListNonBalance.data));
                setDisableSupplier(false);
              }}
              onClear={() => {
                setDisableSupplier(true);
            ...
            <div>
              {!filterStore && <span className="text-red-500 right-0 my-1 sm:mb-0 mb-4">Vui lòng chọn cửa hàng</span>}
            </div>
          </div>
          <div className="flex flex-col">
            <Select
              disabled={disableSupplier}
              className="lg:w-[195px] w-full rounded-[10px]"
  ...
  return (
    <Fragment>
      <div className="revenueByProductStore">
