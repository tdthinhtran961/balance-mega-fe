...
const ImportProductCard = ({ item, dataTempChoose, setDataTempChoose }) => {
  const [showTick, setShowTick] = useState(false);

  const handleChoose = (choosing) => {
    const indexChoose = dataTempChoose?.findIndex(
      (goods) => (goods.productId || goods.idProduct || goods.id) === choosing.id,
    );
    if (indexChoose !== -1) {
      dataTempChoose.splice(indexChoose, 1);
      setShowTick(false);
    } else {
      dataTempChoose.push({
        ...choosing,
        check: true,
        isDeleted: null,
        price: choosing?.priceUnit,
        tax: choosing?.importTax?.taxRate,
        productId: choosing?.id,
        product: {
          barcode: choosing?.barcode,
          basicUnit: choosing?.basicUnit,
          code: choosing?.code,
          id: choosing?.id,
          name: choosing?.name,
          importTax: {
            taxRate: choosing?.importTax?.taxRate,
          },
        },
      });
      setShowTick(true);
