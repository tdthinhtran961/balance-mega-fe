...
const Page = () => {
  ...
  useEffect(() => {
    const fetchDetailOrder = async () => {
      if (idOrder) {
        const data = await SupplierService.getDetailGoodsByCode(idOrder);
        setData(data?.data);
      }
    };
    fetchDetailOrder();
      ...
      <div className="min-h-screen">
        ...
        <div className="bg-white w-full px-4 rounded-xl mt-5 relative pb-5 tableExtend">
          <div className="flex flex-row items-center">
            ...
            {data?.importedStatus === 'PROCESSING' ? (
              <p className="text-[16px] text-yellow-500 py-4 flex flex-row items-center">
            ...
            <div className="w-full flex flex-row text-base">
              ...
              <div className="w-full">
                <div className="w-full flex flex-row mb-5">
                  ...
                  <div>{formatTime(data?.importedAt)}</div>
                  {/* <div>{moment(new Date(data?.createdAt).toLocaleDateString(),'')}</div> */}
                </div>
                {/* {console.log(user?.userInfor?.roleCode)} */}
          ...
          {tabKey === '4' && (
            <div className="flex">
              <div className="w-full flex flex-row">
                <div className="w-full flex mb-5 text-base">
                  <div className="font-bold text-black order">Ngày nhận hàng:</div>
                  <div>{formatTime(data?.deliveredAt, false)}</div>
                </div>
              </div>
              <div className="w-full flex flex-row mb-5 text-base">
          ...
          <hr />
          {
            <>
              {' '}
              <p className="text-[16px] font-bold text-teal-900 py-4 mr-5">Chi tiết nhập hàng</p>
              <table className="table-auto table_discount h-16 w-[98%] mx-5 text-gray-700">
                <thead className="text-left">
                  <tr className="font-normal text-[14px] border-b-[0.5px]">
                    ...
                    <td className="pt-4 pb-2">Đơn giá (VND)</td>
                    <td className="pt-4 pb-2">Số lượng</td>
                    <td className="pt-4 pb-2">Thành tiền (VND)</td>
                    {+tabKey === 3 && (
                      <>
                        <td>Đã nhận</td>
                        <td>Còn lại</td>
                    ...
                    data?.itemList.map((item, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td className="font-normal text-[14px] pt-[7px] pb-2 flex gap-3 items-center">
                            <img
                          ...
                          <td className="font-normal text-[14px] pt-4 pb-6">{item?.basicUnit}</td>
                          <td className="font-normal text-[14px] pt-4 pb-6">{formatCurrency(item?.priceUnit, '')}</td>
                          <td className="font-normal text-[14px] pt-4 pb-6">x{item?.quantity}</td>
              ...
              <div className="flex justify-end mt-[26px] mb-[144px] mr-1">
                <div className="mr-10">
        ...
        <div className="flex justify-between mt-8">
          <button
            onClick={() => {
              // window.history.back();
              navigate(routerLinks('OrderManagement') + `?tabKey=2`);
            }}
